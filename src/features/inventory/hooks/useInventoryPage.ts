...
export function useInventoryPage() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => setPage(1), 0); return () => window.clearTimeout(timer) }, [debouncedSearch, itemType, stockStatus, itemId, quantityMin, quantityMax, periodPreset, customPeriod])
  ...
  const filters = useMemo(() => {
    ...
    return { page, search: debouncedSearch || undefined, item_type: itemType === "all" ? undefined : itemType, stock_status: stockStatus === "all" ? undefined : stockStatus, item_id: itemId ?? undefined, quantity_min: Number.isFinite(min) ? min : undefined, quantity_max: Number.isFinite(max) ? max : undefined, date_from: dateRange?.from, date_to: dateRange?.to }
  }, [page, debouncedSearch, itemType, stockStatus, itemId, quantityMin, quantityMax, dateRange])
