...
export function StatementLedgerTable({ entityType, entries, isLoading, hasSelection }: { entityType: StatementEntityType; entries: StatementEntry[]; isLoading: boolean; hasSelection: boolean }) {
  ...
  return <Table dir="rtl"><TableHeader><HeaderRow /></TableHeader><TableBody>{entries.map((entry, index) => {
    ...
    return <TableRow key={`${entry.entry_type}-${entry.reference_id ?? "opening"}-${entry.entry_date}-${index}`}>
      <TableCell className="whitespace-nowrap text-right text-xs">{formatStatementDate(entry.entry_date)}</TableCell>
      <TableCell className="text-right"><StatementEntryTypeBadge entry={entry} /></TableCell>
      <TableCell className="text-right">{href ? <Button variant="link" size="sm" asChild className="h-auto p-0 font-mono text-xs"><Link href={href} dir="ltr">{reference}</Link></Button> : <span className="inline-block font-mono text-xs" dir="ltr">{reference}</span>}</TableCell>
      ...
      <TableCell className="text-right text-sm">{entry.created_by?.name || "—"}</TableCell>
    </TableRow>
  })}</TableBody></Table>
}

function HeaderRow() {
  return <TableRow><TableHead className="text-right">التاريخ</TableHead><TableHead className="text-right">نوع الحركة</TableHead><TableHead className="text-right">المرجع</TableHead><TableHead className="text-right">البيان</TableHead><TableHead className="text-center">مدين</TableHead><TableHead className="text-center">دائن</TableHead><TableHead className="text-center">الرصيد الجاري</TableHead><TableHead className="text-right">المستخدم</TableHead></TableRow>
}
