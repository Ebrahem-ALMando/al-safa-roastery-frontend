  ...
  return <div className="space-y-6" dir="rtl" lang="ar">
    <DashboardPageHeader>
      ...
      <DashboardPageHeader.Actions>
        <ItemsPeriodControls preset={page.periodPreset} onPresetChange={page.setPeriodPreset} dir="rtl" />
    ...
    {party ? <section className="flex flex-col gap-4 rounded-2xl border bg-linear-to-bl from-primary/10 via-primary/5 to-transparent p-5 text-right sm:flex-row sm:items-center" dir="rtl"><div className="flex size-12 shrink-0 items-center justify-center rounded-xl border bg-card text-primary"><Scale className="size-6" /></div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-start gap-2"><h2 className="text-lg font-bold">{party.name}</h2><StatementPartyBadge party={party} /></div><p className="mt-1 text-right font-mono text-xs text-muted-foreground" dir="ltr">{party.code || `#${party.id}`}{party.phone ? ` · ${party.phone}` : ""}</p></div><div className="shrink-0 text-right sm:text-left"><p className="text-xs text-muted-foreground">الرصيد الحالي المسجل</p><p className="mt-1 text-xl font-bold" dir="ltr">{formatStatementMoney(party.current_balance)}</p><p className="text-xs text-muted-foreground">{statementBalanceMeaning(party.type, party.current_balance)}</p></div></section> : null}

    {page.selectedEntity ? <StatementSummary entityType={page.entityType} summary={page.statement?.summary} isLoading={page.isLoading} /> : null}

    {page.error && !page.isLoading ? <div className="flex flex-col items-center gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-6 text-right" dir="rtl"><p className="text-sm text-destructive">{STATEMENT_MESSAGES.loadingError}</p><Button variant="outline" onClick={() => void page.mutate()} className="gap-2"><RefreshCw className="size-4" />إعادة المحاولة</Button></div>
