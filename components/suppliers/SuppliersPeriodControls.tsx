...
import {
  SUPPLIERS_PERIOD_DROPDOWN_PRESETS,
  SUPPLIERS_PERIOD_LABELS_AR,
  type SuppliersPeriodPreset,
} from "@/features/suppliers/lib/suppliers.constants"
...
type Props = {
  ...
  onPresetChange: (preset: SuppliersPeriodPreset) => void
  className?: string
}

export function SuppliersPeriodControls({ preset, onPresetChange, className }: Props) {
  const allActive = preset === "all"
  const dropdownActive = !allActive
  const triggerLabel = allActive
    ? "الفترة"
    : SUPPLIERS_PERIOD_LABELS_AR[preset]
  ...
  return (
    <div className={cn("flex shrink-0 items-center gap-1.5 sm:gap-2", className)}>
      ...
      >
        ...
        <span className="truncate">{SUPPLIERS_PERIOD_LABELS_AR.all}</span>
      </Button>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="min-w-44">
          {SUPPLIERS_PERIOD_DROPDOWN_PRESETS.map((p) => (
            <DropdownMenuItem
              key={p}
              className="cursor-pointer justify-end text-right"
              onClick={() => onPresetChange(p)}
            >
              {SUPPLIERS_PERIOD_LABELS_AR[p]}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
