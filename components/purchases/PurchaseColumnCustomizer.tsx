...
interface PurchaseColumnCustomizerProps {
  visibleColumns: PurchaseTableColumnId[]
  onChange: (columns: PurchaseTableColumnId[]) => void
}

export function PurchaseColumnCustomizer({
  visibleColumns,
  onChange,
}: PurchaseColumnCustomizerProps) {
  const visibleSet = new Set(visibleColumns)
  const [dragIndex, setDragIndex] = useState<number | null>(null)
  const [dragOverIndex, setDragOverIndex] = useState<number | null>(null)

  const hiddenColumns = PURCHASE_TABLE_COLUMNS.filter((c) => !visibleSet.has(c.id))

  function toggleColumn(id: PurchaseTableColumnId) {
    const col = PURCHASE_TABLE_COLUMNS.find((c) => c.id === id)
    if (col?.essential) return

    if (visibleSet.has(id)) {
      onChange(normalizePurchaseVisibleColumns(visibleColumns.filter((c) => c !== id)))
      return
    }
    onChange(insertPurchaseColumnBeforeActions(visibleColumns, id))
  }

  function showAll() {
    onChange(normalizePurchaseVisibleColumns(PURCHASE_TABLE_COLUMNS.map((c) => c.id)))
  }

  function hideAll() {
    onChange(normalizePurchaseVisibleColumns(["invoice_number", "actions"]))
  }

  function restoreDefault() {
    onChange(DEFAULT_VISIBLE_PURCHASE_COLUMNS)
  }

  function reorderColumns(from: number, to: number) {
    if (from === to) return
    const col = PURCHASE_TABLE_COLUMNS.find((c) => c.id === visibleColumns[from])
    ...
    onChange(normalizePurchaseVisibleColumns(next))
  }
      ...
      <DropdownMenuContent align="end" dir="rtl" className="w-72 text-right">
        ...
        <div className="max-h-72 space-y-1 overflow-y-auto px-2 py-1">
          {visibleColumns.map((id, index) => {
            const col = PURCHASE_TABLE_COLUMNS.find((c) => c.id === id)
            const canDrag = col?.id !== "actions" && col?.id !== "invoice_number"
            return (
              <div
                key={id}
                draggable={canDrag}
              ...
              >
                <Checkbox
                  ...
                  className="shrink-0"
                />
                <span className="min-w-0 flex-1 truncate text-right">{getPurchaseColumnLabel(id)}</span>
                {canDrag ? (
