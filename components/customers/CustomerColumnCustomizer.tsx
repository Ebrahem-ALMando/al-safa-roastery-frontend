...
interface CustomerColumnCustomizerProps {
  visibleColumns: CustomerTableColumnId[]
  onChange: (columns: CustomerTableColumnId[]) => void
}

export function CustomerColumnCustomizer({
  visibleColumns,
  onChange,
}: CustomerColumnCustomizerProps) {
  const visibleSet = new Set(visibleColumns)
  const [dragIndex, setDragIndex] = useState<number | null>(null)
  const [dragOverIndex, setDragOverIndex] = useState<number | null>(null)

  const hiddenColumns = CUSTOMER_TABLE_COLUMNS.filter((c) => !visibleSet.has(c.id))

  function toggleColumn(id: CustomerTableColumnId) {
    const col = CUSTOMER_TABLE_COLUMNS.find((c) => c.id === id)
    if (col?.essential) return

    if (visibleSet.has(id)) {
      onChange(normalizeCustomerVisibleColumns(visibleColumns.filter((c) => c !== id)))
      return
    }
    onChange(insertCustomerColumnBeforeActions(visibleColumns, id))
  }

  function showAll() {
    onChange(normalizeCustomerVisibleColumns(CUSTOMER_TABLE_COLUMNS.map((c) => c.id)))
  }

  function hideAll() {
    onChange(normalizeCustomerVisibleColumns(["customer_name", "actions"]))
  }

  function restoreDefault() {
    onChange(DEFAULT_VISIBLE_CUSTOMER_COLUMNS)
  }

  function reorderColumns(from: number, to: number) {
    if (from === to) return
    const col = CUSTOMER_TABLE_COLUMNS.find((c) => c.id === visibleColumns[from])
    ...
    onChange(normalizeCustomerVisibleColumns(next))
  }
      ...
      <DropdownMenuContent align="end" dir="rtl" className="w-72 text-right">
        ...
        <div className="max-h-72 space-y-1 overflow-y-auto px-2 py-1">
          {visibleColumns.map((id, index) => {
            const col = CUSTOMER_TABLE_COLUMNS.find((c) => c.id === id)
            const canDrag = col?.id !== "actions" && col?.id !== "customer_name"
            return (
              <div
                key={id}
                draggable={canDrag}
              ...
              >
                <Checkbox
                  ...
                  className="shrink-0"
                />
                <span className="min-w-0 flex-1 truncate text-right">{getCustomerColumnLabel(id)}</span>
                {canDrag ? (
