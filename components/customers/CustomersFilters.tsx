...
import type { CustomersActiveStatus, CustomersTypeFilter } from "@/features/customers"
import type { BalanceRangeDirection, BalanceStatusFilter } from "@/features/customers"
import { CUSTOMER_TYPE_LABELS_AR } from "@/features/customers"

export interface CustomersFiltersValue {
  search: string
  isActive: CustomersActiveStatus
  customerType: CustomersTypeFilter
...
interface CustomersFiltersProps {
  ...
  onChange: (value: CustomersFiltersValue) => void
  isLoading?: boolean
}
...
export function CustomersFilters({ value, onChange, isLoading = false }: CustomersFiltersProps) {
  ...
  const customerTypeLabel =
    value.customerType !== "all" ? CUSTOMER_TYPE_LABELS_AR[value.customerType] : undefined

  const balanceStatusLabel =
    value.balanceStatus === "receivable"
      ? "عليه لنا"
    ...
    <div className="flex flex-col gap-4 rounded-xl border bg-card p-4 shadow-sm">
      ...
      >
        <div className="grid grid-cols-1 gap-4 border-t border-border/60 pb-2 pt-4 md:grid-cols-5 md:items-end">
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(selected) =>
                onChange({
                  ...value,
                  isActive: selected as CustomersActiveStatus,
                })
              }
              disabled={isLoading}
            >
              <SelectTrigger id="customers-filter-status" className="h-10 w-full">
          ...
          <div className="space-y-2">
            <Label htmlFor="customers-filter-type" className="text-xs text-muted-foreground">
              نوع الزبون
            </Label>
            <Select
              ...
              onValueChange={(selected) =>
                onChange({
                  ...value,
                  customerType: selected as CustomersTypeFilter,
                })
              }
              disabled={isLoading}
            >
              <SelectTrigger id="customers-filter-type" className="h-10 w-full">
          ...
          <div className="space-y-2">
            <Label htmlFor="customers-filter-balance-status" className="text-xs text-muted-foreground">
              حالة الرصيد
            </Label>
            <Select
              ...
              disabled={isLoading}
            >
              <SelectTrigger id="customers-filter-balance-status" className="h-10 w-full">
                <SelectValue placeholder="الكل" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">الكل</SelectItem>
                <SelectItem value="receivable">عليه لنا</SelectItem>
                <SelectItem value="credit">له عندنا</SelectItem>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
          ...
          {customerTypeLabel ? (
            <Badge variant="secondary" className="bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-200">
              ...
              <button
                ...
                onClick={() => onChange({ ...value, customerType: "all" })}
              >
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ...
          {hasBalanceRange ? (
            <Badge variant="secondary" className="bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-200">
              ...
              {value.balanceRangeDirection === "receivable"
                ? " (عليه لنا)"
