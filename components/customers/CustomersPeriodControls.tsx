...
import {
  CUSTOMERS_PERIOD_DROPDOWN_PRESETS,
  CUSTOMERS_PERIOD_LABELS_AR,
  type CustomersPeriodPreset,
} from "@/features/customers/lib/customers.constants"
...
type Props = {
  ...
  onPresetChange: (preset: CustomersPeriodPreset) => void
  className?: string
}

export function CustomersPeriodControls({ preset, onPresetChange, className }: Props) {
  const allActive = preset === "all"
  const dropdownActive = !allActive
  const triggerLabel = allActive ? "الفترة" : CUSTOMERS_PERIOD_LABELS_AR[preset]
  ...
  return (
    <div className={cn("flex shrink-0 items-center gap-1.5 sm:gap-2", className)}>
      ...
      >
        ...
        <span className="truncate">{CUSTOMERS_PERIOD_LABELS_AR.all}</span>
      </Button>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="min-w-44">
          {CUSTOMERS_PERIOD_DROPDOWN_PRESETS.map((p) => (
            <DropdownMenuItem
              key={p}
              className="cursor-pointer justify-end text-right"
              onClick={() => onPresetChange(p)}
            >
              {CUSTOMERS_PERIOD_LABELS_AR[p]}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
