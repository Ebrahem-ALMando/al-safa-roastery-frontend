...
import { LayoutGrid, ListChecks, RefreshCw, Settings2, Table } from "lucide-react"
...
export function InventoryView() {
  ...
  return <div className="space-y-6" dir="rtl" lang="ar">
    <DashboardPageHeader>
      <DashboardPageHeader.Lead><h1 className="flex items-center gap-2 text-md font-bold">المستودع / <span className="font-normal text-muted-foreground">إدارة الأرصدة وحركات المخزون</span></h1></DashboardPageHeader.Lead>
      <DashboardPageHeader.Actions>
        ...
        <Button variant="outline" asChild className="gap-2 rounded-xl"><Link href="/dashboard/inventory/movements"><ListChecks className="size-4" />عرض كل الحركات</Link></Button>
      </DashboardPageHeader.Actions>
    </DashboardPageHeader>
    ...
    <InventoryWithdrawalDialog key={withdrawItem?.id ?? "withdrawal-closed"} item={withdrawItem} open={withdrawItem !== null} onOpenChange={(open) => { if (!open) setWithdrawItem(null) }} />
    <InventoryAdjustmentDialog key={adjustItem?.id ?? "adjustment-closed"} item={adjustItem} open={adjustItem !== null} onOpenChange={(open) => { if (!open) setAdjustItem(null) }} />
  </div>
}
